...
export default function FreeSignalRoom() {
  const [messages, setMessages] = useState<Message[]>([])
  const [isAvailable, setIsAvailable] = useState(true)
  const { user } = useAuth()

  useEffect(() => {
    // Check if admin has toggled availability
    const fetchAvailability = async () => {
      const settingsRef = doc(firestore, 'config', 'free_signals')
  ...
  useEffect(() => {
    if (!user?.uid || !isAvailable) return
    ...
    return () => unsubscribe()
  }, [user?.uid, isAvailable])

  if (!isAvailable) {
    return (
      <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md text-center">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">Free Signal Room</h2>
        <p className="text-gray-500 italic">
          Free signals are not available right now. Please check back later.
        </p>
      </div>
    )
  }
